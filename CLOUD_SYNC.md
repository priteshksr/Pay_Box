...
-- 11) Realtime — publish the tables so postgres_changes events fire.
...
2. The worker opens the link → lands on the green **PayBox · Worker login**
   screen.
3. They enter their **phone number + invite code** and tap **Continue**.
   ...
   - Calls `claim_staff_invite(code, displayName?, phone)` which marks the
     code as used, links the anonymous user to the staff record, and
     seeds the phone onto the staff profile via a `staff_update` event.
...
> **No SMS, no password.** The invite code IS the proof of identity — it's
> owner-issued, one-shot, and revocable. If a worker loses their device or
> needs to sign in on a new one, the owner taps **Rotate invite** on that
> staff record and reshares the new code.
